...
def addDerived(data):
    ...
    data['s1z'] = data.a1*data.cost1
    data['s2z'] = data.a2*data.cost2
    data['s1p'] = data.a1*np.sqrt(1.-data.cost1**2)
    data['s2p'] = data.a2*np.sqrt(1.-data.cost2**2)

def input_output(data):

    input_data = data[['Mc_over_d','log_Mtot','eta','s1z','s2z','s1p','s2p','cos_inclination','right_ascension','sin_declination','polarization']]
...
def run_training(output):
    ...
    ann = build_ann(input_shape=11,layer_width=64,hidden_layers=3,lr=1e-4)
