...
def addDerived(data):
    ...
    data['log_Mc'] = np.log(data.Mc_detector)
    data['log_Mtot'] = np.log(data.Mc_detector/data.eta**(3./5.))
    data['sin_declination'] = np.sin(data.declination)

def input_output(data):

    input_data = data[['Mc_over_d','log_Mc','log_Mtot','Xeff','Xp_gen','cos_inclination','right_ascension','sin_declination','polarization']]
...
def run_training(job):
    ...
    ann = build_ann(layer_width=64,hidden_layers=3,lr=1e-4)
    ...
    with open("/project2/kicp/tcallister/learning-p-det-data/param-study/logMc_logM/{0:02d}.json".format(job),'w') as jf:
        json.dump(stats,jf)

if __name__=="__main__":

    jobnumber = int(sys.argv[1])
    run_training(jobnumber)
