...
def run_training(output):
    ...
    with open('{0}_scaler.json'.format(output),'w') as jf:
        json.dump(scaler_params,jf)    

    ann = build_ann(input_shape=10,layer_width=64,hidden_layers=3,lr=1e-4,leaky_alpha=0.001)
    callbacks = [tf.keras.callbacks.LearningRateScheduler(new_scheduler, verbose=0)]
    ...
    with open("{0}_output_stats.json".format(output),'w') as jf:
        json.dump(stats,jf)
